...
import { ServeLength, ServeCourse, Course3Way, OpponentHand } from '@/types/table-tennis';

interface TableTennisCourtProps {
  mode: 'serve_select' | 'course_3way_select' | 'heatmap_serve' | 'heatmap_3way';
  ...
  title?: string;
  opponentHand?: OpponentHand; // 右利き / 左利き
}
...
export const TableTennisCourt: React.FC<TableTennisCourtProps> = ({
  ...
  // ユーザー要件に基づくコース配置:
  // 相手コートを上から見た場合 (相手が右利きの場合、自分から見て左側が相手フォア、右側が相手バック):
  // [上段・奥 (ロング)]: フォアロング | フォアミドルロング | バックミドルロング | バックロング
  // [下段・ネット前 (前)]: フォア前 | フォアミドル前 | バックミドル前 | バック前
  const courses: ServeCourse[] = opponentHand === 'left'
    ? ['back', 'back_middle', 'fore_middle', 'fore'] // 左利き時はバックが左、フォアが右
    : ['fore', 'fore_middle', 'back_middle', 'back']; // 右利き時はフォアが左、バックが右
      ...
      {title && (
        <div className="text-center font-bold text-slate-800 text-xs sm:text-sm mb-2 flex items-center justify-center gap-1.5">
          ...
          <span className="text-[9px] sm:text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded-full border border-slate-200 font-normal">
            相手: {opponentHand === 'left' ? '左利き' : '右利き'}視点
          </span>
        </div>
      )}
      ...
      <div className="relative bg-blue-900 border-2 sm:border-4 border-slate-300 rounded-xl p-1.5 sm:p-2.5 shadow-inner overflow-hidden">
        ...
        {(mode === 'serve_select' || mode === 'heatmap_serve') && (
          <div className="space-y-1.5">
            {/* 上段: ロング (フォアロング / フォアミドルロング / バックミドルロング / バックロング) */}
            ...
            {/* 下段: 前 (フォア前 / フォアミドル前 / バックミドル前 / バック前) */}
            <div className="grid grid-cols-4 gap-1.5">
              {courses.map((crs) => {
